...
namespace VE
{
	std::expected<std::vector<char>, B32> Pipeline::ReadFile(const std::string& filePath)
	{
		...
		const PipelineConfigInfo& configInfo)
	{
		...
		if (!vertCode)
		{
			std::println("Failed to read Vert shader!");
			return;
		}
		auto fragCode = ReadFile(fragFilePath);
		if (!fragCode)
		{
			std::println("Failed to read frag shader!");
			return;
		}
		...
		auto bindingDescs = Model::Vertex::GetBindingsDescriptions();
		auto attributeDescs = Model::Vertex::GetAttributeDescriptions();
		...
		{
			std::println("Failed to create the graphics pipeline!");
			return;
		}
	...
	PipelineConfigInfo Pipeline::DefaultPipelineConfigInfo(U32 width, U32 height)
	{
		...
		//configInfo.colorBlendInfo.pNext = ?;
